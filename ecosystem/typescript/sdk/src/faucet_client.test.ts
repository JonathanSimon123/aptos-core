...
test(
  ...
  async () => {
    ...
    const events = await client.getEventsByEventHandle(tx.sender, "0x1::Coin::CoinStore", "withdraw_events");
    expect(events[0].type).toBe("0x1::Coin::WithdrawEvent");

    const event_subset = await client.getEventsByEventHandle(tx.sender, "0x1::Coin::CoinStore", "deposit_events", {
      start: 0,
      limit: 1,
    });
    expect(event_subset[0].type).toBe("0x1::Coin::DepositEvent");
